...
func TestCodexStartBuildsCodexCLICompatibleAuthorizeURL(t *testing.T) {
	...
	if !strings.Contains(u.RawQuery, "api.connectors.read%20api.connectors.invoke") {
		t.Fatalf("codex authorize query should request current connector scopes, got raw query: %s", u.RawQuery)
	}
