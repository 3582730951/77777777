...
const baseURL = "https://api.openblocklabs.com/v1/chat/completions"
...
func New(mode string) *Provider {
	...
	return &Provider{
		Mode:       mode,
		httpClient: transport.ForProvider("api.openblocklabs.com", transport.Options{Timeout: 120 * time.Second}),
	}
}

func (p *Provider) SetStore(s *store.Store) { p.store = s }

func (p *Provider) Name() string { return "openblocklabs" }
...
func (p *Provider) Probe(ctx context.Context, acc *domain.Account) error {
	...
	if err != nil {
		return fmt.Errorf("openblocklabs probe: %w", err)
	}
	r, err := http.NewRequestWithContext(ctx, http.MethodGet, "https://api.openblocklabs.com/v1/models", nil)
	if err != nil {
		return err
	}
	...
	if resp.StatusCode >= 400 {
		return fmt.Errorf("openblocklabs probe: status %d", resp.StatusCode)
	}
	return nil
}
...
func (p *Provider) Discover(ctx context.Context, acc *domain.Account) (*domain.QuotaState, error) {
	models := []domain.ModelCapability{
		{ID: "obl-default", SupportsTools: false, SupportsVision: false},
	}
...
func (p *Provider) invokeReal(ctx context.Context, acc *domain.Account, req *ir.Request) (<-chan ir.Event, error) {
	sec, err := p.store.GetAccountSecret(ctx, acc.ID)
	if err != nil {
		return nil, fmt.Errorf("openblocklabs: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("openblocklabs invoke: %w", err)
	}
	if resp.StatusCode >= 400 {
		b, _ := io.ReadAll(resp.Body)
		resp.Body.Close()
		return nil, fmt.Errorf("openblocklabs invoke: status %d: %s", resp.StatusCode, string(b))
	}
...
func (p *Provider) invokeMock(ctx context.Context, acc *domain.Account, req *ir.Request) (<-chan ir.Event, error) {
	...
	go func() {
		...
		response := fmt.Sprintf(
			"[mock openblocklabs provider · account=%s · model=%s]\n\nYou said: %q\n\nMock response from openblocklabs provider.",
