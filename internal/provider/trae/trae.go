...
const baseURL = "https://api-sg-central.trae.ai/cloudide/api/v3/trae/Chat"
...
func New(mode string) *Provider {
	...
	return &Provider{
		Mode:       mode,
		httpClient: transport.ForProvider("api-sg-central.trae.ai", transport.Options{Timeout: 120 * time.Second}),
	}
}

func (p *Provider) SetStore(s *store.Store) { p.store = s }

func (p *Provider) Name() string { return "trae" }
...
func (p *Provider) Probe(ctx context.Context, acc *domain.Account) error {
	...
	if err != nil {
		return fmt.Errorf("trae probe: %w", err)
	}
	// Best-effort probe: send a minimal request
	body, _ := json.Marshal(map[string]any{
		"model":    "claude-sonnet-4-6",
		"messages": []map[string]string{{"role": "user", "content": "hi"}},
		"stream":   false,
	})
	r, err := http.NewRequestWithContext(ctx, http.MethodPost, baseURL, bytes.NewReader(body))
	if err != nil {
		return err
	}
	...
	if resp.StatusCode >= 400 {
		return fmt.Errorf("trae probe: status %d", resp.StatusCode)
	}
	return nil
}
...
func (p *Provider) invokeReal(ctx context.Context, acc *domain.Account, req *ir.Request) (<-chan ir.Event, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("trae invoke: %w", err)
	}
	if resp.StatusCode >= 400 {
		b, _ := io.ReadAll(resp.Body)
		resp.Body.Close()
		return nil, fmt.Errorf("trae invoke: status %d: %s", resp.StatusCode, string(b))
	}
...
func (p *Provider) invokeMock(ctx context.Context, acc *domain.Account, req *ir.Request) (<-chan ir.Event, error) {
	...
	go func() {
		...
		response := fmt.Sprintf(
			"[mock trae provider · account=%s · model=%s]\n\nYou said: %q\n\nMock response from trae provider.",
