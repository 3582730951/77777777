...
def _wait_for_token(page, timeout: int = 120, log_fn=None) -> str:
    ...
    while time.time() < deadline:
        ...
        if tok:
            return tok
        cur_url = page.url
        if cur_url != last_url and log_fn:
            log_fn(f"[diag] URL变化: {cur_url[:120]}")
            last_url = cur_url
        time.sleep(1)
    return ""
...
""")
            ...
            while time.time() < turnstile_deadline:
                ...
                if pwd and pwd.is_visible():
                    self.log("已跳转到密码页，Turnstile 自动通过")
                    passed_already = True
                    break
                time.sleep(1)

            # 检测 invisible Turnstile：没有可见 iframe 但页面卡在 spinner
            if not has_turnstile and not passed_already:
                invisible_ts = page.query_selector('input[name="cf-turnstile-response"]')
                if invisible_ts:
                    self.log("检测到 Turnstile invisible 模式（页面卡在 spinner）")
                    has_turnstile = True
            ...
            if has_turnstile:
                ...
                if not solved:
                    ...
                    if token:
                        ...
                        _click_continue(page)
                        time.sleep(5)
                        # invisible 模式注入后可能需要等待页面跳转
                        pwd_wait = time.time() + 15
                        while time.time() < pwd_wait:
                            pwd = page.query_selector('input[type="password"]')
                            if pwd and pwd.is_visible():
                                break
                            otp = page.query_selector('input[name="otp"], input[name="code"]')
                            if otp and otp.is_visible():
                                break
                            time.sleep(1)
                    else:
                        self.log("自动解题失败，等待手动通过（最多90秒）...")
                        dl = time.time() + 90
                        while time.time() < dl:
                            if not _is_turnstile_modal_visible(page):
                                break
            ...
            if _handle_turnstile(page, self.log, self._solve_turnstile):
                # Turnstile 通过后需要再次提交密码表单
                time.sleep(1)
                _click_continue(page)
                time.sleep(3)
            ...
            # --- OTP 提交后可能出现手机号验证（Cursor 新流程） ---
            self.log(f"OTP提交后 URL: {page.url}")
            if "radar-challenge" in page.url or "phone" in page.url.lower():
                self.log("检测到手机号验证页（OTP后）")
            # 也尝试检测页面内容
            try:
                phone_input_post = page.query_selector('input[type="tel"]')
                if not phone_input_post:
                    page.wait_for_selector('input[type="tel"]', timeout=5000)
                    phone_input_post = page.query_selector('input[type="tel"]')
            except Exception:
                phone_input_post = None

            if phone_input_post and phone_input_post.is_visible():
                if self.phone_callback:
                    phone_number = self.phone_callback()
                    if phone_number:
                        self.log(f"填写手机号: {str(phone_number)[:4]}****")
                        phone_input_post.click()
                        phone_input_post.fill(str(phone_number).strip())
                        time.sleep(0.5)
                        ...
                        try:
                            ...
                            self.log(f"等待短信验证码失败: {e}")
                    else:
                        raise RuntimeError("phone_callback 未返回手机号")
                else:
                    raise RuntimeError(
                        "Cursor 注册需要手机号验证（OTP后），但未配置 phone_callback / SMS provider。"
                    )

            # 等待 Session Token
            self.log("等待 WorkosCursorSessionToken")
            tok = _wait_for_token(page, timeout=60, log_fn=self.log)
            if not tok:
                self.log(f"最终 URL: {page.url}")
                try:
                    body_preview = page.evaluate("() => document.body?.innerText?.slice(0,300) || ''")
                    self.log(f"页面内容: {body_preview[:200]}")
                except Exception:
                    pass
