...
class KiroProtocolMailboxWorker:
    ...
    def run(
        ...
        *,
        ...
        # Pass OTP callback for browser-based device auth login
        if otp_callback:
            self.client._otp_callback = lambda e, timeout=120: otp_callback()
        elif mail_token:
            self.client._otp_callback = lambda e, timeout=120: wait_for_otp(mail_token, timeout=timeout, tag=self.client.tag)
        ...
        # If we got a refreshToken from ExchangeToken Set-Cookie, use it directly (Social auth mode)
        if tokens.get("refreshToken"):
            self.client.log("🎉 注册完成! (含 accessToken + refreshToken, Social auth 模式)")
            return {
                "email": email,
        ...
        # Otherwise try OIDC device auth for refreshToken
        # Use ssoSessionToken (from Set-Cookie) if available - it's the actual SSO bearer
        bearer_token = tokens.get("ssoSessionToken") or tokens.get("ssoBearerToken") or tokens.get("awsBearerToken") or tokens["sessionToken"]
