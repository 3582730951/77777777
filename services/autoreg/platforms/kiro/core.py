...
"""
import os,re,uuid,json,random,string,time,base64,hashlib,secrets
...
_SENSITIVE_LOG_KEYS = {
    ...
    "session_token", "sessiontoken", "sso_session_token", "ssosessiontoken",
    "bearer_token", "bearertoken", "csrf_token", "csrftoken",
    "client_secret", "clientsecret", "password", "authorization", "cookie",
    "code", "authcode", "code_verifier", "codeverifier",
}
def _redact_sensitive_for_log(value):
    if isinstance(value, dict):
        out = {}
        for k, v in value.items():
            key_norm = str(k).replace("-", "_").lower()
            key_compact = key_norm.replace("_", "")
            if key_norm in _SENSITIVE_LOG_KEYS or key_compact in _SENSITIVE_LOG_KEYS:
                out[k] = _mask_secret(v)
            else:
                out[k] = _redact_sensitive_for_log(v)
        return out
    if isinstance(value, list):
        return [_redact_sensitive_for_log(v) for v in value]
    return value
def _safe_json_for_log(value, limit=300):
    try:
        text = json.dumps(_redact_sensitive_for_log(value), ensure_ascii=False, default=str)
    except Exception:
        text = str(value)
    return text[:limit]
def _describe_set_cookie(header_value):
    first_part = str(header_value).split(";", 1)[0].strip()
    if "=" not in first_part:
        return "<malformed>"
    name, value = first_part.split("=", 1)
    return f"{name.strip()}={_mask_secret(value.strip())}"
def _safe_url_for_log(value, limit=140):
...
def _single_query_value(query, name):
    ...
    if len(values) != 1:
        return None
    return values[0]
def _append_private_jsonl(path, record):
...
class KiroRegister:
    def __init__(self,proxy=None,tag="REG"):
        ...
        self._step1_redirect_url=None   # step 1 返回的 OIDC authorize URL
    ...
    def _validate_oauth_state(self, redirect_state):
        if not redirect_state:
            self.log("  ❌ OAuth 回调缺少 state")
            return False
        if redirect_state != self.state:
            self.log(f"  ❌ OAuth state 不匹配: got={_mask_secret(redirect_state)} expected={_mask_secret(self.state)}")
            return False
        return True
    ...
    def _exec(self,step_id,inputs=None,prefix="",action_id=None,
        ...
        self.log(f"  Resp: {_safe_json_for_log(d, 400)}")
        return d
    ...
    def step2_get_wsh(self,redir_url):
        ...
        if wsh_direct:
            ...
            # ★ 主动获取 portal csrfToken（Step 12 需要）
            try:
                pu = "https://portal.sso.us-east-1.amazonaws.com/login?directory_id=view"
                r_portal = self.s.get(pu, headers={**UA, "accept": "*/*",
    ...
    def step3_signin_flow(self,email):
        ...
        if redir:
            self.log(f"  ✅ signup redirect: {_safe_url_for_log(redir)}")
            m=re.search(r"workflowStateHandle=([^&#]+)",redir)
    ...
    def step4_signup_flow(self,email):
        ...
        if "profile.aws" in redir:
            self.log(f"  ✅ profile redirect: {_safe_url_for_log(redir)}")
    ...
    def step5_get_tes_token(self):
        ...
        if r.status_code==200:
            ...
            self.log(f"  ✅ awsd2c-token: {_mask_secret(token)}")
    ...
    def step8_create_identity(self, otp, email, full_name):
        ...
        if not reg_code or not sign_in_state:
            ...
            return None
        self.log(f"  ✅ registrationCode: {_mask_secret(reg_code)}")
        try:
    ...
    def step9_signup_registration(self, reg_code, sign_in_state):
        ...
        self.log(f"  9a: GET {_safe_url_for_log(signup_url)}")
        ...
        for name,val,dom,path in self._safe_cookie_list("signin.aws"):
            self.log(f"    {name}={_mask_secret(val)} (domain={dom}, path={path})")
    ...
    def step11_final_login(self, email, step10_resp):
        ...
        if not redir:
            ...
            return None
        self.log(f"  redirect: {_safe_url_for_log(redir)}")
        p = urlparse(redir)
        qs = parse_qs(p.query)
    ...
    def step12_get_tokens(self):
        ...
        # 检查必要数据
        has_sso_data = bool(self._portal_csrf_token and self._workflow_result_handle and self._step11_state)

        bearer_token = None
        sso_redirect = None

        # ── 12a: 尝试 POST portal.sso/auth/sso-token（旧路径） ──
        if has_sso_data:
            ...
            if r.status_code == 200:
                ...
                self.log(f"  ⚠️ sso-token 失败: {r.status_code}, body={r.text[:300]}")
                self.log(f"  ⚠️ authCode={_mask_secret(self._workflow_result_handle)}, state={_mask_secret(self._step11_state)}")

        # ── 12a-alt: sso-token 失败或数据不全，手动跟踪 OIDC redirect chain ──
        if not bearer_token and self._step1_redirect_url:
            self.log("  12a-alt: 手动跟踪 OIDC redirect chain...")
            current_url = self._step1_redirect_url
            auth_code = None
            redirect_state = None
            sso_bearer_from_chain = None
            for hop in range(20):
                r_hop = self.s.get(current_url, headers={**UA, "accept": "text/html"}, allow_redirects=False)
                loc = r_hop.headers.get("location", "")
                self.log(f"  [{hop}] {r_hop.status_code} {_safe_url_for_log(current_url, 80)} → {_safe_url_for_log(loc, 80) if loc else '(no redirect)'}")
                # 捕获 portal.sso 设置的 session token（Set-Cookie 里的 x-amz-sso-token）
                for h_name, h_val in r_hop.headers.items():
                    if h_name.lower() == "set-cookie":
                        self.log(f"  [{hop}] Set-Cookie: {_describe_set_cookie(h_val)}")
                        if "x-amz-sso-token" in h_val.lower() or "sso-token" in h_val.lower():
                            import http.cookies
                            c = http.cookies.SimpleCookie()
                            c.load(h_val)
                            for morsel_name, morsel in c.items():
                                if "sso" in morsel_name.lower() and "token" in morsel_name.lower():
                                    sso_bearer_from_chain = morsel.value
                                    self.log(f"  [{hop}] 捕获 SSO token: {morsel_name}={_mask_secret(morsel.value)}")
                # 检查是否到了 kiro.dev 带 code
                target = loc or str(getattr(r_hop, 'url', current_url))
                if "kiro.dev" in target and "code=" in target:
                    p_t = urlparse(target)
                    qs_t = parse_qs(p_t.query)
                    auth_code = _single_query_value(qs_t, "code")
                    redirect_state = _single_query_value(qs_t, "state")
                    if not auth_code or not redirect_state:
                        self.log("  ❌ OAuth redirect 参数数量不正确")
                        return None
                    self.log(f"  [{hop}] ✅ 拿到 auth code: {_mask_secret(auth_code)}")
                    break
                if r_hop.status_code not in (301, 302, 303, 307):
                    # 检查 response body 里有没有 token
                    try:
                        body_data = r_hop.json()
                        t = body_data.get("token", "")
                        if t and not sso_bearer_from_chain:
                            sso_bearer_from_chain = t
                            self.log(f"  [{hop}] 捕获 bearer token from body: {_mask_secret(t)}")
                    except Exception:
                        pass
                    break
                current_url = loc
                if current_url and not current_url.startswith("http"):
                    p_prev = urlparse(self._step1_redirect_url)
                    current_url = f"{p_prev.scheme}://{p_prev.netloc}{current_url}"
            if sso_bearer_from_chain:
                self._sso_bearer_token = sso_bearer_from_chain
            if auth_code:
                return self._step12e_exchange_token(auth_code, redirect_state)
            # 如果到了 signin.aws 的 login 页（session 没生效），尝试 follow redirect
            elif "signin.aws" in final_url and "workflowStateHandle" in final_url:
                self.log("  AWS SSO session 未生效，尝试再走一轮 login workflow...")
                # 直接用当前 session cookies 重新执行 login workflow
                p_f = urlparse(final_url)
                qs_f = parse_qs(p_f.query)
                new_wsh = qs_f.get("workflowStateHandle", [None])[0]
                if new_wsh:
                    url_exec = f"{SIGNIN}/platform/{DIR_ID}/api/execute"
                    h_exec = {**UA, "accept": "application/json", "content-type": "application/json",
                              "origin": SIGNIN, "referer": f"{SIGNIN}/platform/{DIR_ID}/login"}
                    # init
                    r1 = self.s.post(url_exec, headers=h_exec, json={
                        "stepId": "", "workflowStateHandle": new_wsh, "inputs": [], "requestId": _uuid()})
                    d1 = r1.json()
                    self.log(f"  re-login init: sid={d1.get('stepId')}")
                    if d1.get("stepId") == "end-of-workflow-success":
                        redir2 = d1.get("redirect", {}).get("url", "")
                        if redir2:
                            r2 = self.s.get(redir2, headers={**UA, "accept": "text/html"}, allow_redirects=True)
                            final2 = str(r2.url)
                            if "kiro.dev" in final2 and "code=" in final2:
                                p2 = urlparse(final2)
                                qs2 = parse_qs(p2.query)
                                auth_code = _single_query_value(qs2, "code")
                                redirect_state = _single_query_value(qs2, "state")
                                if not auth_code or not redirect_state:
                                    self.log("  ❌ re-login OAuth redirect 参数数量不正确")
                                    return None
                                if auth_code:
                                    self.log(f"  ✅ re-login 拿到 auth code: {_mask_secret(auth_code)}")
                                    return self._step12e_exchange_token(auth_code, redirect_state)
            self.log("  ❌ 直接重定向方式也未获取到 auth code")
            return None

        if not bearer_token:
            self.log("  ❌ 无法获取 bearer token")
            return None
        sso_resp = r.json()
        bearer_token = sso_resp.get("token", "")
        sso_redirect = sso_resp.get("redirectUrl", "")
        if not bearer_token:
            self.log(f"  ❌ 无 bearer token: {_safe_json_for_log(sso_resp, 300)}")
            return None
        self.log(f"  ✅ bearer token (sessionToken)={_mask_secret(bearer_token)}")
        self.log(f"  redirectUrl={_safe_url_for_log(sso_redirect)}")
        ...
        if r.status_code == 200:
            try:
                whoami = r.json()
                self.log(f"  ✅ whoAmI: {_safe_json_for_log(whoami, 200)}")
            except: pass
        ...
        if not auth_location:
            self.log(f"  ❌ 无 location: {_safe_json_for_log(ar_resp, 300)}")
            return None
        self.log(f"  ✅ location={_safe_url_for_log(auth_location)}")
        ...
        if not redirect_loc:
            ...
            return None
        self.log(f"  ✅ redirect → {_safe_url_for_log(redirect_loc)}")

        # 从 redirect URL 提取 code 和 state
        p_loc = urlparse(redirect_loc)
        qs_loc = parse_qs(p_loc.query)
        auth_code = _single_query_value(qs_loc, "code")
        redirect_state = _single_query_value(qs_loc, "state")
        if not auth_code:
            self.log(f"  ❌ redirect URL 中 code 参数缺失或数量不正确")
            return None
        if not self._validate_oauth_state(redirect_state):
            return None
        self.log(f"  ✅ auth_code={_mask_secret(auth_code)}")
        self.log(f"  ✅ redirect_state={_mask_secret(redirect_state)}")
        ...
        if not access_token:
            self.log(f"  ❌ 无 accessToken: {_safe_json_for_log(resp_data)}")
            return None
        self.log(f"  ✅ accessToken={_mask_secret(access_token)}")
        self.log(f"  ✅ csrfToken={_mask_secret(kiro_csrf)}")
    ...
    def _device_auth_via_browser(self, verification_uri, user_code):
        """用 Camoufox 浏览器自动完成 AWS SSO 设备授权确认。

        流程：先在浏览器里通过 portal.sso 登录建立 SSO session，
        然后打开 device verification 页面点击确认。
        """
        try:
            from camoufox.sync_api import Camoufox
        except ImportError:
            self.log("  ❌ Camoufox 未安装")
            return False

        # 从 curl_cffi session 提取 cookies（Step 1-11 积累的）
        cookies_to_inject = []
        for c in self.s.cookies.jar:
            domain = c.domain or ""
        ...
        self.log(f"  注入 {len(cookies_to_inject)} 个 cookies")

        try:
            with Camoufox(headless=True) as browser:
                ctx = browser.new_context()
                # ★ 不注入旧 cookies — 让浏览器从零登录，避免过期 session 干扰
                page = ctx.new_page()

                # 在浏览器里完整走一遍 AWS Builder ID 登录，建立 SSO session
                # 注意：这需要注册时的邮箱和密码（存在 self._last_email / self._last_password）
                reg_email = getattr(self, '_last_email', '')
                reg_password = getattr(self, '_last_password', '')
                if not reg_email or not reg_password:
                    self.log("  ❌ 缺少邮箱/密码，无法在浏览器中登录")
                    page.close()
                    return False

                self.log(f"  在浏览器中用 {reg_email} 登录 AWS Builder ID...")
                login_url = "https://view.awsapps.com/start"
                page.goto(login_url, timeout=30000, wait_until="domcontentloaded")
                time.sleep(5)

                # ★ 关闭 Cookie 同意弹窗 (AWS 页面会显示此弹窗)
                for sel in ['button:has-text("Accept")', 'button:has-text("接受")', '#awsccc-cb-btn-accept']:
                    try:
                        btn = page.query_selector(sel)
                        if btn and btn.is_visible():
                            btn.click()
                            self.log("  ✅ 关闭 Cookie 弹窗")
                            time.sleep(2)
                            break
                    except Exception:
                        continue
                ...
                for _ in range(15):
                    ...
                    if "cookie" not in body.lower()[:100] and len(body.strip()) > 30:
                        break
                    time.sleep(1)

                current_url = page.url
                self.log(f"  当前 URL: {_safe_url_for_log(current_url)}")
                ...
                if "signin.aws" in current_url:
                    ...
                    # ★ 先关闭 signin.aws 上的 Cookie 弹窗
                    for sel in ['button:has-text("Accept")', '#awsccc-cb-btn-accept']:
                        try:
                            btn = page.query_selector(sel)
                            if btn and btn.is_visible():
                                btn.click()
                                self.log("  ✅ 关闭 signin.aws Cookie 弹窗")
                                time.sleep(2)
                                break
                        except Exception:
                            continue

                    # 填写邮箱 (AWS 登录页用 input[type="text"], 不是 email)
                    try:
                        page.wait_for_selector('input[type="text"], input[name="username"], input[type="email"]', timeout=10000)
                    except Exception:
                        pass
                    email_input = page.query_selector('input[name="username"]') or page.query_selector('input[type="email"]') or page.query_selector('input[type="text"]')
                    if email_input and email_input.is_visible():
                        email_input.fill(reg_email)
                        self.log(f"  ✅ 填写邮箱: {reg_email}")
                        time.sleep(1)
                        # 点 Next/Submit/Continue
                        for sel in ['button[type="submit"]', 'button:has-text("Next")', 'button:has-text("Continue")', 'button:has-text("继续")']:
                            btn = page.query_selector(sel)
                            if btn and btn.is_visible():
                                btn.click()
                                self.log(f"  ✅ 点击: {sel}")
                    ...
                    if pwd_input and pwd_input.is_visible():
                        pwd_input.fill(reg_password)
                        self.log("  ✅ 填写密码")
                        time.sleep(1)
                        for sel in ['button[type="submit"]', 'button:has-text("Sign in")', 'button:has-text("Continue")', 'button:has-text("Next")', 'button:has-text("继续")']:
                            btn = page.query_selector(sel)
                            if btn and btn.is_visible():
                                btn.click()
                                self.log(f"  ✅ 点击: {sel}")
                    ...
                    # ★ 检查是否需要 OTP 验证 (Verify your identity)
                    body_after_pwd = page.evaluate("() => document.body?.innerText || ''")
                    if "Verify" in body_after_pwd or "verification" in body_after_pwd.lower() or "digit" in body_after_pwd.lower():
                        self.log("  📧 需要登录 OTP 验证，等待验证码...")
                        otp_callback = getattr(self, '_otp_callback', None)
                        if otp_callback:
                            try:
                                otp_code = otp_callback(reg_email, timeout=120)
                                if otp_code:
                                    self.log(f"  ✅ 收到 OTP: {otp_code}")
                                    otp_input = page.query_selector('input[type="text"]') or page.query_selector('input[placeholder*="digit"]')
                                ...
                                self.log(f"  ❌ OTP 获取失败: {e}")
                        else:
                            self.log("  ⚠️ 无 OTP 回调，尝试继续...")

                    self.log(f"  登录后 URL: {_safe_url_for_log(page.url)}")

                    # ★ 登录成功后，先访问 view.awsapps.com/start 建立 SSO session
                    self.log("  建立 SSO session...")
                    page.goto("https://view.awsapps.com/start", timeout=15000, wait_until="domcontentloaded")
                    time.sleep(5)
                    self.log(f"  SSO session URL: {_safe_url_for_log(page.url)}")

                # 打开 device 授权页面
                device_url = f"https://view.awsapps.com/start/#/device?user_code={user_code}"
                self.log(f"  打开 device 页面: {device_url[:80]}...")
                page.goto(device_url, timeout=20000, wait_until="domcontentloaded")
                time.sleep(3)
                ...
                for _ in range(20):
                    body = page.evaluate("() => document.body?.innerText || ''")
                    if "cookie" not in body.lower()[:100] and "Privacy" not in body.strip() and len(body.strip()) > 30:
                        break
                    time.sleep(1)

                # ★ 处理多个授权确认页面 (参考 kiro-auto-register)
                # 页面1: "Confirm this code matches" → Confirm and continue
                # 页面2: "Allow xxx to access your data?" → Allow access
                # 页面3: "Request approved" → 完成
                auth_completed = False
                for auth_attempt in range(10):
                    time.sleep(3)
                    body_text = page.evaluate("() => document.body?.innerText || ''")
                    self.log(f"  [{auth_attempt}] 页面: {body_text[:150]}")

                    if "Request approved" in body_text or "successfully" in body_text.lower():
                        self.log("  ✅ 设备授权完成!")
                        auth_completed = True
                        break

                    if "Confirm this code" in body_text or "Authorization requested" in body_text:
                        self.log("  📌 设备确认页面")
                        for sel in ['button:has-text("Confirm and continue")', 'button:has-text("Confirm")', 'button[type="submit"]']:
                            try:
                                btn = page.query_selector(sel)
                                if btn and btn.is_visible():
                                    btn.click()
                    ...
                    if "Allow" in body_text and ("access your data" in body_text or "Kiro" in body_text):
                        self.log("  📌 应用授权页面")
                        for sel in ['button:has-text("Allow access")', 'button:has-text("Allow")', 'button[type="submit"]']:
                            try:
                                btn = page.query_selector(sel)
                                if btn and btn.is_visible():
                                    btn.click()
                    ...
                    # 如果是空白页或Privacy页，可能登录未生效
                    if len(body_text.strip()) < 30 or body_text.strip() == "Privacy":
                        self.log("  ⚠️ 页面空白/未加载，等待...")
                        continue

                    # 可能需要先输入 code
                    code_input = page.query_selector('input[type="text"]')
                    if code_input and code_input.is_visible():
                        self.log(f"  输入 code: {user_code}")
                        code_input.fill(user_code)
                        time.sleep(0.5)
                        for sel in ['button:has-text("Next")', 'button:has-text("Verify")', 'button[type="submit"]']:
                            try:
                                btn = page.query_selector(sel)
                                if btn and btn.is_visible():
                                    btn.click()
                ...
                if not auth_completed:
                    self.log("  ❌ 设备授权未完成")
                    page.screenshot(path="/tmp/kiro_device_auth_fail.png")
                    page.close()
                    return False
    ...
    def _step12e_exchange_token(self, auth_code, redirect_state):
        """12e: POST ExchangeToken (CBOR) — 用 auth code + PKCE codeVerifier 换 accessToken"""
        if not auth_code:
            self.log("  ❌ ExchangeToken 缺少 auth code")
            return None
        if not self._validate_oauth_state(redirect_state):
            return None
        self.log("  12e: POST ExchangeToken (CBOR)...")
        ...
        self.s.cookies.set("kiro-visitor-id", self.vid, domain="app.kiro.dev", path="/")
        r = self.s.post(
            f"{KIRO}/service/KiroWebPortalService/operation/ExchangeToken",
            headers=exchange_h, data=exchange_body)
        self.log(f"  Status: {r.status_code}")
        ...
        for h_name, h_val in r.headers.items():
            if h_name.lower() == "set-cookie":
                self.log(f"  Set-Cookie: {_describe_set_cookie(h_val)}")
        if r.status_code != 200:
            self.log(f"  ❌ ExchangeToken 失败: {r.status_code}")
            try: self.log(f"  {r.text[:500]}")
            except: pass
        ...
        if not access_token:
            self.log(f"  ❌ 无 accessToken: {_safe_json_for_log(resp_data)}")
            return None
        # ★ 从 Set-Cookie 提取 refreshToken 和 SessionToken
        # 注意: curl_cffi 的 headers.items() 会合并同名头，必须用 get_list/multi_items
        refresh_token_from_cookie = ""
        session_token_from_cookie = ""
        sc_list = r.headers.get_list("set-cookie") if hasattr(r.headers, "get_list") else []
        if not sc_list:
            sc_list = [v for k,v in (r.headers.multi_items() if hasattr(r.headers,"multi_items") else r.headers.items()) if k.lower()=="set-cookie"]
        for sc_val in sc_list:
            part = sc_val.split(";")[0].strip()
            if "=" in part:
                ck, cv = part.split("=", 1)
                ck_s = ck.strip()
                cv_s = cv.strip()
                if ck_s in ("RefreshToken","refreshToken") and cv_s:
                    refresh_token_from_cookie = cv_s
                    self.log(f"  ★ refreshToken from Set-Cookie: {_mask_secret(refresh_token_from_cookie)}")
                elif ck_s == "SessionToken" and cv_s:
                    session_token_from_cookie = cv_s
                    self.log(f"  ★ SessionToken from Set-Cookie: {_mask_secret(session_token_from_cookie)}")
        self.log(f"  ✅ accessToken={_mask_secret(access_token)} csrfToken={_mask_secret(kiro_csrf)} expiresIn={expires_in}")

        # ── 12e2: 用 csrfToken + session 调 GetCredentials 获取 AWS OIDC token ──
        aws_bearer = ""
        if kiro_csrf:
            self.log("  12e2: POST GetCredentials (获取 AWS bearer token)...")
            gc_h = {
                **UA,
                "accept": "application/cbor",
                "content-type": "application/cbor",
                ...
                "origin": KIRO,
                "referer": f"{KIRO}/",
                "x-kiro-csrf-token": kiro_csrf,
                "x-amz-user-agent": "aws-sdk-js/1.0.0 ua/2.1 os/macOS lang/js md/browser#Chromium_131 m/N,M,E",
                "amz-sdk-invocation-id": _uuid(),
                "amz-sdk-request": "attempt=1; max=1",
            }
            self.s.cookies.set("kiro-session", access_token, domain="app.kiro.dev", path="/")
            gc_r = self.s.post(
                f"{KIRO}/service/KiroWebPortalService/operation/GetCredentials",
                headers=gc_h, data=cbor2.dumps({}))
            self.log(f"  GetCredentials status: {gc_r.status_code}")
            if gc_r.status_code == 200:
                try:
                    gc_data = cbor2.loads(gc_r.content)
                    aws_bearer = gc_data.get("bearerToken", "") or gc_data.get("accessToken", "") or gc_data.get("token", "")
                    self.log(f"  GetCredentials keys: {list(gc_data.keys())}")
                    if aws_bearer:
                        self.log(f"  ✅ AWS bearer token: {_mask_secret(aws_bearer)}")
                    else:
                        self.log(f"  ⚠️ GetCredentials 无 bearerToken: {_safe_json_for_log(gc_data, 300)}")
                except Exception as e:
                    self.log(f"  ❌ GetCredentials CBOR解析失败: {e}")
            else:
                try:
                    err = cbor2.loads(gc_r.content)
                    self.log(f"  ❌ GetCredentials: {_safe_json_for_log(err, 200)}")
                except:
                    self.log(f"  ❌ GetCredentials: {gc_r.text[:200]}")

        result = {
            "accessToken": access_token,
            "sessionToken": session_token_from_cookie or access_token,
            "csrfToken": kiro_csrf,
            "expiresIn": expires_in,
        }
        if refresh_token_from_cookie:
            result["refreshToken"] = refresh_token_from_cookie
        if session_token_from_cookie:
            result["ssoSessionToken"] = session_token_from_cookie
        if aws_bearer:
            result["awsBearerToken"] = aws_bearer
        if getattr(self, '_sso_bearer_token', ''):
            result["ssoBearerToken"] = self._sso_bearer_token
        return result
    ...
    def step12f_device_auth(self, bearer_token):
        ...
        if not client_id or not client_secret:
            ...
            return None
        self.log(f"  ✅ clientId={_mask_secret(client_id)}")
        ...
        self.log(f"  ✅ verificationUri={_safe_url_for_log(verification_uri)}")
        ...
        # 12h: 设备授权确认
        # 尝试协议方式，失败则回退到浏览器自动化
        session_id = bearer_token or getattr(self, '_sso_bearer_token', '') or ''
        device_auth_ok = False

        if session_id:
            # 12h-1: 协议方式 POST accept_user_code
            self.log(f"  12h-1: POST accept_user_code (协议方式)...")
            r = self.s.post(
                f"{OIDC}/device_authorization/accept_user_code",
                headers=oidc_h,
                json={"userCode": user_code, "userSessionId": session_id})
            if r.status_code == 200:
                accept_resp = r.json()
                ...
                # 12h-2 ~ 12h-4
                r2 = self.s.post(f"{PORTAL}/session/device",
                    headers={**UA, "accept": "application/json, text/plain, */*",
                             ...
                             "referer": "https://view.awsapps.com/"},
                    json={})
                device_token = r2.json().get("token", session_id) if r2.status_code == 200 else session_id
                self.s.post(f"{OIDC}/consent_details", headers=oidc_h,
                    json={"deviceContextId": dc_id,
                          "clientId": device_context.get("clientId", client_id),
                          "clientType": device_context.get("clientType", "public"),
                          "userSessionId": device_token})
                r4 = self.s.post(f"{OIDC}/device_authorization/associate_token", headers=oidc_h,
                    json={"deviceContext": device_context, "userSessionId": device_token})
                device_auth_ok = r4.status_code in (200, 204)
                if device_auth_ok:
                    self.log(f"  ✅ 协议方式设备授权完成")
            else:
                self.log(f"  ⚠️ 协议 accept_user_code 失败: {r.status_code}")

        if not device_auth_ok:
            # 12h-browser: 浏览器自动化方式完成设备授权
            self.log(f"  12h-browser: 启动 Camoufox 完成设备授权...")
            device_auth_ok = self._device_auth_via_browser(
                verification_uri or f"https://view.awsapps.com/start/#/device?user_code={user_code}",
                user_code,
            )
            if device_auth_ok:
                self.log(f"  ✅ 浏览器设备授权完成")
            else:
                self.log(f"  ❌ 浏览器设备授权也失败")
                return None
        ...
        self.log(f"  ✅ OIDC accessToken={_mask_secret(oidc_access)}")
        self.log(f"  ✅ refreshToken={_mask_secret(refresh_token)}")
        return {
            "clientId": client_id,
            "clientSecret": client_secret,
...
def main():
    ...
    if ok:
        ...
        print(f"  密码: {_mask_secret(info['password'])}")
        if info.get('accessToken'):
            print(f"  accessToken: {_mask_secret(info['accessToken'])}")
            print(f"  sessionToken: {_mask_secret(info['sessionToken'])}")
        if info.get('refreshToken'):
            print(f"  clientId: {_mask_secret(info['clientId'])}")
            print(f"  clientSecret: {_mask_secret(info['clientSecret'])}")
            print(f"  refreshToken: {_mask_secret(info['refreshToken'])}")
        _append_private_jsonl("kiro_accounts.txt", {
            "email": info['email'],
