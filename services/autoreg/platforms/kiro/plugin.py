...
class KiroPlatform(BasePlatform):
    ...
    def _map_kiro_result(self, result: dict, *, password: str = "", oauth_provider: str = "") -> RegistrationResult:
        # Auto-enroll to gateway if we have OIDC credentials
    ...
    def _auto_enroll_gateway(self, email: str, refresh_token: str, client_id: str, client_secret: str):
        ...
        try:
            ...
            gateway_url = os.environ.get("GATEWAY_ADMIN_URL", "http://127.0.0.1:9090")
            r = cffi.post(
                f"{gateway_url}/api/admin/kiro/enroll",
                json={
            ...
            if r.status_code in (200, 201):
                logging.getLogger(__name__).info(f"[Kiro] Auto-enrolled {email} to gateway: {r.json()}")
            else:
                logging.getLogger(__name__).warning(f"[Kiro] Gateway enroll failed: {r.status_code} {r.text[:100]}")
        except Exception as e:
            logging.getLogger(__name__).warning(f"[Kiro] Gateway auto-enroll error: {e}")
