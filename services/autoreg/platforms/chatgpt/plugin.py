...
class ChatGPTPlatform(BasePlatform):
    ...
    capabilities = [
        ...
        "auto_upgrade",     # Auto upgrade to Plus via PayPal
    ]
    ...
    def get_platform_actions(self) -> list:
        return [
            ...
            {"id": "auto_upgrade", "label": "自动升级 Plus",
             "params": [
                 {"key": "card_info", "label": "卡信息 (卡号 ---- 过期 ---- CVV ---- 3DS手机 ---- 3DS SMS API ---- 姓名 ---- 地址)", "type": "text"},
                 {"key": "paypal_info", "label": "PayPal (手机|SMS API)", "type": "text"},
             ]},
        ]
    ...
    def _execute_platform_action(self, action_id: str, account: Account, params: dict) -> dict:
        ...
        if action_id == "auto_upgrade":
            ...
            return {
                "ok": result.success,
                "data" if result.success else "error": {
