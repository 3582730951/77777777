...
class RegistrationEngine:
    ...
    def _send_verification_code(self) -> bool:
        ...
        try:
            ...
            referer = "https://auth.openai.com/email-verification" if self._is_existing_account else "https://auth.openai.com/create-account/password"
            response = self.session.get(
    ...
    def run(self) -> RegistrationResult:
        ...
        try:
            ...
            # 判断是否需要先验证邮箱再设密码（新流程）
            _otp_before_password = signup_result.page_type in (
                "email_otp_verification_registration",
                "email_otp_send",
            )

            # 8. 如果需要先验证邮箱（新流程），先发 OTP → 验证 → 再设密码
            if _otp_before_password:
                self._log("8. [新流程] 先验证邮箱再设密码...")
                self._log("8a. 发送验证码...")
                if not self._send_verification_code():
                    self._log("send_otp 返回失败，尝试继续等待", "warning")
                    self._otp_sent_at = time.time()
                ...
                # OTP 验证后可能需要创建账户
                if self._otp_page_type in ("about_you", "create_account_password"):
                    self._log("8e. 创建用户账户...")
                ...
                # 已注册账号：跳过密码，直接发 OTP 验证
                self._log("8. [已注册账号] 跳过密码设置")

                self._log("9. 发送验证码...")
                if not self._send_verification_code():
                    self._log("send_otp 失败，可能已自动发送，继续等待", "warning")
                    self._otp_sent_at = time.time()
                ...
                # 12. 根据 OTP 响应决定下一步
                if self._otp_page_type in ("about_you",):
                    self._log("12. 创建用户账户...")
                    if not self._create_user_account():
                        result.error_message = "创建用户账户失败"
                        return result
            ...
            try:
                ...
                self._log(f"session keys: {list(session_data.keys())}, accessToken 长度: {len(access_token)}")
            except Exception:
                self._log(f"session API 响应非 JSON: {session_resp.text[:200]}", "warning")

            if access_token:
                self._log("NextAuth session 获取成功")
            else:
                self._log("NextAuth session 未返回 accessToken，将通过 Codex CLI 获取", "warning")
