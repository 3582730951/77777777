...
class AnythingProtocolMailboxWorker:
    def __init__(self, *, proxy: str | None = None, log_fn: Callable[[str], None] = print, captcha_solver=None):
    ...
    def run(
        ...
        *,
        ...
        if self._captcha_solver:
            try:
                from platforms.anything.core import ANYTHING_BASE, ANYTHING_TURNSTILE_SITEKEY
                self.log("获取 Anything Turnstile token...")
                turnstile_token = self._captcha_solver.solve_turnstile(ANYTHING_BASE + "/signup", ANYTHING_TURNSTILE_SITEKEY)
                if turnstile_token:
                    self.log(f"Turnstile token: {turnstile_token[:40]}...")
            except Exception as exc:
                self.log(f"Turnstile 自动获取失败: {exc}")
