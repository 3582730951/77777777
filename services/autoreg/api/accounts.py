...
def list_accounts(
    ...
    return service.list_accounts(AccountQuery(platform=platform, status=status, email=email, synced=synced, page=page, page_size=page_size))
...
@router.patch("/{account_id}/synced")
def mark_synced(account_id: int):
    from infrastructure.accounts_repository import AccountsRepository
    repo = AccountsRepository()
    if not repo.mark_synced(account_id):
        raise HTTPException(404, "账号不存在")
    return {"ok": True}
