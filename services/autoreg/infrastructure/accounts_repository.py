...
class AccountsRepository:
    ...
    def list(self, query: AccountQuery) -> tuple[int, list[AccountRecord]]:
        ...
        with Session(engine) as session:
            ...
            if query.synced in ("true", "false"):
                statement = statement.join(
                    AccountOverviewModel,
                    AccountModel.id == AccountOverviewModel.account_id,
                    isouter=True,
                )
                if query.synced == "false":
                    from sqlalchemy import or_
                    statement = statement.where(or_(
                        AccountOverviewModel.synced_to_pool == False,
                        AccountOverviewModel.synced_to_pool.is_(None),
                    ))
                else:
                    statement = statement.where(AccountOverviewModel.synced_to_pool == True)
    ...
    def mark_synced(self, account_id: int) -> bool:
        from datetime import datetime, timezone
        with Session(engine) as session:
            overview = session.get(AccountOverviewModel, account_id)
            if not overview:
                return False
            overview.synced_to_pool = True
            overview.synced_at = datetime.now(timezone.utc)
