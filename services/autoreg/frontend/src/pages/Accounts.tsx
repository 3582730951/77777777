...
import { apiDownload, apiFetch, triggerBrowserDownload, copyToClipboard } from '@/lib/utils'
...
function ActionMenu({
      ...
      .then(resp => {
        if (resp?.sync) {
          ...
          if (resp.data?.url || resp.data?.checkout_url || resp.data?.cashier_url) {
            ...
            void copyToClipboard(actionUrl).catch(() => {})
          }
  ...
  const handleActionDone = async (status: string) => {
    ...
    try {
      ...
      if (actionUrl) {
        window.open(actionUrl, '_blank')
        await copyToClipboard(actionUrl).catch(() => {})
      }
